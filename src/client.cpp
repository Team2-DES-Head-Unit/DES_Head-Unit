...
void receive_gear_state()
{
    ...
    std::cout << "Waiting for connection from Python...\n";
    new_socket = accept(server_fd, (sockaddr *)&address, &al);
    if (new_socket < 0) { perror("accept"); return; }

    while (true) {
        int n = read(new_socket, &control, sizeof(control));
        if (n <= 0) break;

        if (control.gear_P) gear_state = "P";
        else if (control.gear_D) gear_state = "D";
        else if (control.gear_R) gear_state = "R";
        else if (control.gear_N) gear_state = "N";

        std::cout << "Gear: "      << gear_state
                  << " | Throttle: " << control.throttle
                  << " | Steering: " << control.steering
                  << " | Speed: "    << control.speed     << " cm/s"
                  << " | Distance: " << control.distance  << " cm\n";
    }
